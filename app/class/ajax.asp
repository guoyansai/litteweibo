...
Function delLog()
	...
	Dim I1,I2,A1
    Set Rs=Conn.Execute("select pic,pics from ["&DBsn&"ai] where id="&id)
	I1=Rs("pic")
	I2=Rs("pics")
	...
	If I2<>"" Then 
	  ...
	  Next 
	End if
    Conn.Execute("delete from ["&DBsn&"ai] where id="&id)
...
Function  addLog()   
    ...
    If logs<>"" Then
	   ...
	   If Trim(summ)="" Then summ = "#分享"
        Conn.Execute("insert into ["&DBsn&"ai](title,summ,content,fm,pic,pics,pass,hide,lock)values('"&tit&"','"&summ&"','"&logs&"','"&fm&"','"&pic&"','"&pics&"','"&pas&"',"&hide&","&lock&")") 
		'Response.Write "ok" 
...
Function editLog()    
    ...
    If logs<>"" Then
        ...
        Conn.Execute("update ["&DBsn&"ai] set title='"&tit&"',summ='"&summ&"',content='"&logs&"',pic='"&pic&"',pics='"&pics&"',atime='"&atime&"',pass='"&pas&"',hide="&hide&",lock="&lock&" where id="&id)
    End If
...
Sub plsave()
    ...
    If rlog<>"" Then
        Conn.Execute("update ["&DBsn&"pl] set rcontent='"&rlog&"',isn=0 where pid="&pid)
    End If
...
Function zdLog()
    ...
    Conn.Execute("update ["&DBsn&"ai] set ist="&x&" where id="&id)
	If x=1 then	 
	  Call Message("200","取消","")
	Else	  
	  Call Message("200","置顶","")
...
Function shPl()
	...
	Dim pid
	pid = CLng(Request.Querystring("pid"))
    Conn.Execute("update ["&DBsn&"pl] set isn=0 where pid="&pid)
	Application.Contents.Remove("Com_List"&Flag) '删除缓存
...
Function delpl()
	...
	Dim pid
	pid = CLng(Request.Querystring("pid"))
    Conn.Execute("delete from ["&DBsn&"pl] where pid="&pid)
	Conn.Execute("update ["&DBsn&"ai] set num=num-1 where id="&id)
	Application.Contents.Remove("Com_List"&Flag)
...
Sub addPl()
	...
	Set ooRs = Conn.Execute("select id,hide,lock from ["&DBsn&"ai] where id=" & id)
    ...
    If plog<>"" And pname<>"" Then
        Conn.Execute("insert into ["&DBsn&"pl](cid,pname,pcontent,isn)values("&id&",'"&pname&"','"&plog&"',"&I1&")")	    
		Conn.Execute("update ["&DBsn&"ai] set num=num+1 where id="&id)
		Response.Cookies(CookSn&"nick") = pname
	    Response.Cookies(CookSn&"nick").Expires = Date()+7
    End If	
...
Function saveSet()
	...
	If swebuser<>"" And swebtitle<>"" And swebdesc<>"" Then 
		Conn.Execute("update ["&DBsn&"wz] set webuser='"&swebuser&"',webtitle='"&swebtitle&"',webdesc='"&swebdesc&"',icp='"&icp&"',plsh="&splsh&",safecode="&ssafecode&",menu='"&smenu&"',rewrite="&srewrite&" where id=1")
		'Conn.Execute("update ["&DBsn&"wz] set rewrite="&srewrite&" where id=1")
...
Function saveWid() 
	...
	If title<>"" And html<>"" And ord<>"" Then
	  If id = 0 Then
	    Conn.Execute("insert into ["&DBsn&"wd] (title,html,ord)values('"&title&"','"&html&"',"&ord&")")
	    Call Message("200","添加侧栏成功！","")
	  Else 
		Conn.Execute("update ["&DBsn&"wd] set title='"&title&"',html='"&html&"',ord="&ord&" where id="&id)
		Application.Contents.Remove("Web_Set"&Flag) '删除缓存
...
Function delWid() 
     Call isLogin()
	 'Call Message("200","设置更新成功！演示系统不更新","")
     'Conn.Execute("delete * from  ["&DBsn&"wd] where id="&id)
...
Function ckPass()
	...
	Set Rs=Conn.Execute("select pass,content from ["&DBsn&"ai] where id="&id)
	If ps = Rs(0) Then
	    txt = Replace(Rs(1),"""","\""")
