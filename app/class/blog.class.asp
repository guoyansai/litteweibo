...
Private Sub Class_Initialize
	...
	sRs.Open "select * from ["&DBsn&"wd] order by ord",Conn,1,1
...
Public Sub run()
	...
	Select Case action
		...
		Case "set"
			Call isLogin()
			Set oRs  = Conn.Execute("select * from ["&DBsn&"wz] where id=1")
			templete = "setting.htm"
			tpl.assign "v",oRs
		Case "wid"
			Call isLogin()
			'Set oRs = Conn.Execute("select * from ["&DBsn&"wd]")
			templete = "widget.htm"
		...
		Case "edit"
			...
			Set oRs  = Conn.Execute("select * from ["&DBsn&"ai] where id=" & id)
			templete = "post.htm"
			tpl.assign "v",oRs
			tpl.assign "c",action
			tpl.assign "btn","编 辑"
		...
		Case "pl"
			...
			'Set cRs = Conn.Execute("select * from ["&DBsn&"pl] where cid="&id)
		
			If admin = 1 Then
				nick = WebUser
			Else
				nick = Request.Cookies(CookSn&"nick")
			End If

			if oRs("lock")=0 Then
			Set cRs  = Server.CreateObject("ADODB.RecordSet")
			cRs.Open "select * from ["&DBsn&"pl] where cid=" & id,Conn,1,1
			tpl.assign "list",cRs
