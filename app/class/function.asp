...
Function NewCom(num)
		...
		Application.Lock()
		Set Rs=Conn.Execute("Select top "&num&" pid,cid,pname,pcontent,rcontent from ["&DBsn&"pl] where isn=0 order by pid desc")
		If Rs.Eof Then 
		Rs.Close
		NewCom = "<li>暂无评论！</li>"
		Exit Function
...
Function TopIc(num)
		...
		Application.Lock()
		Set Rs=Conn.Execute("Select top "&num&" id,title,summ,num from ["&DBsn&"ai] where hide=0 order by num desc,atime desc")
		If Rs.Eof Then 
		Rs.Close
		TopIc = "<li>暂无评论！</li>"
		Exit Function
...
Function pFrom(id)
	...
	If admin = 1 Then 
	  I1=WebUser
	Else
	  I1=Request.Cookies(CookSn&"nick")
	End If
	If SafeCode = 1 Then
...
Function doLogin()
    If Pass = Request.Form("pass") Then
		Response.Cookies(CookSn&Flag)=pass
		Response.Cookies(CookSn&Flag).Expires = Date()+7
		'Response.Cookies(CookSn&Flag).domain=Domain
    End If
	Response.Redirect("./")
	Response.End()	
End Function
'退出登录
Function logOut()
    Response.Cookies(CookSn&Flag)=""		
	Response.Cookies(CookSn&Flag).Expires = Now()-1
	'Response.Cookies(CookSn&Flag).domain=Domain
	Response.Redirect("./")
	Response.End()	
End Function
